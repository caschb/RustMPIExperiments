...
+ [[https://gitlab.com/CNCA_CeNAT/bs-solctra-repos/bs-solctra-mpi-rs][BS-Solctra-mpi-rs]]: My Rust+Rayon+MPI implementation of BS-Solctra
...
* LTO comparison (2026-09-07)

Follow-up for the CLEI 2026 talk: the deck says LTO was off on both sides and
attributes the PIC gap to un-inlined RSMPI wrappers. This section rebuilds
every binary twice, with and without link-time optimisation, using the same
compilers, and runs the four variants back to back inside one allocation so
the comparison does not depend on the 2025 numbers. Everything lives in
=lto/= (scripts, job outputs, =results.csv=); figures come from =make lto= in
=viz/=.

** Toolchain

The spack =rustdev= environment no longer exists, so the Rust toolchain is
pinned with rustup and =libclang= (needed by =mpi-sys= for bindgen) comes from
a micromamba environment. The C/C++ side is unchanged: =mpich/3.3.2-gcc-9.3.0=
and =gcc/9.3.0= modules. GCC 9 only has =-flto= (no =-flto=auto=).

#+begin_src bash
rustup toolchain install 1.85.0 --profile minimal
export MAMBA_ROOT_PREFIX=/data/casch/tools/mamba
srun --partition=kura --cpus-per-task=4 --time=0-00:30:00 \
  /data/casch/tools/bin/micromamba create -y -p /data/casch/tools/envs/clang -c conda-forge libclang
#+end_src

** Build

Run behind =srun= on a kura node (=lto/build-lto.sh=). The recorded 2025
flags are kept as the baseline; LTO is the only change. Note that the paper
says =-O3 -march=native= and =target-cpu=native=, but neither was in the
recorded builds, so neither is used here.

#+begin_src bash
. /opt/Modules/3.2.10/init/sh; module purge; module load mpich/3.3.2-gcc-9.3.0 gcc/9.3.0
export LIBCLANG_PATH=/data/casch/tools/envs/clang/lib

# C++ BS-SOLCTRA (bs-solctra-implementations)
SRC="solctra_multinode.h solctra_multinode.cpp main_multinode.cpp utils.h utils.cpp"
mpicxx -O3 -std=c++11 -fopenmp -o bs-solctra-multinode $SRC
mpicxx -O3 -std=c++11 -fopenmp -flto -o bs-solctra-multinode-lto $SRC

# C PIC (Kernels/MPI1/PIC-static)
make pic OPTFLAGS="-O3 -mtune=native -ffast-math -g3 -Wall -flto" && cp pic pic-lto
make clean; make pic OPTFLAGS="-O3 -mtune=native -ffast-math -g3 -Wall"

# Rust (both Cargo.toml files gained this profile; codegen-units left at default)
#   [profile.release-lto]
#   inherits = "release"
#   lto = "fat"
cargo +1.85.0 build --release
cargo +1.85.0 build --profile release-lto
#+end_src

LTO was verified from verbose output: =cargo build -v= shows =-C lto=fat= on
the two LTO binaries only, and =mpicc -v= / =mpicxx -v= invoke =lto-wrapper=
and =lto1= with =liblto_plugin.so=. Binary sizes: C++ 84 KB vs 64 KB,
C PIC 222 KB vs 218 KB, Rust PIC 1.28 MB vs 1.05 MB, Rust BS-SOLCTRA 3.02 MB
vs 2.49 MB. The 2025 binaries are kept in =lto/orig-binaries-2025/=.

** Smoke test

One node (job 612035): all 16 PIC variant/style combinations print
"Solution validates"; the BS-SOLCTRA output files of the LTO binaries are
byte-identical to the no-LTO ones for both languages. Both codes only write
the step-0 file (print period 10000), so this checks input handling, not the
final positions.

** Runs

One job per node count on =kura-wide=, =--exclusive=,
=--ntasks-per-node=20 --cpus-per-task=1=, =--mail-type=END,FAIL=
(=lto/run-lto-{1,2,4}.slurm=, helpers in =lto/pic-common.sh=). Every
repetition runs C, C+LTO, Rust, Rust+LTO back to back, preceded by a marker
line =### code=... lang=... lto=... scaling=... style=... rep=...= on stdout
and stderr. Command lines are those of =sol-mpi-st/4=, =sol-rst-st/4=,
=pic-*-st-p/4= and =pic-*-wk-p/4=; the one difference is that the Rust
BS-SOLCTRA reads its CSV coils from a =resources-rs= symlink because the two
codes' resource formats differ.

| Job    | Nodes | Elapsed | Limit | Content                                                          |
|--------+-------+---------+-------+------------------------------------------------------------------|
| 612036 |     1 | 3:02:55 | 4 h   | PIC strong+weak, 4 styles, 10 reps; BS-SOLCTRA strong, 5 reps    |
| 612037 |     2 | 1:44:04 | 2 h   | same                                                             |
| 612038 |     4 | 1:18:33 | 1.5 h | same                                                             |

PIC strong: 12*102400 particles; weak: SLURM_NTASKS*102400; grid 1000,
100 steps, pure MPI via MPICH =mpirun -n $SLURM_NTASKS=. BS-SOLCTRA strong:
61440 particles, 1000 steps, =srun -n $SLURM_NNODES= with 20 OpenMP/Rayon
threads. All 960 PIC runs validated; no errors on stderr.

** Parse and plot

#+begin_src bash :dir lto
python3 parse-results.py .     # -> results.csv (code,lang,lto,nodes,ranks,scaling,style,rep,metric)
python3 analyze.py results.csv # mean/sd, MAPE, LTO speedup per configuration
#+end_src

#+begin_src bash :dir viz
make lto   # figures/pic_lto_rates.pdf pic_lto_speedup.pdf pic_lto_mape.pdf solctra_lto.pdf
#+end_src

** Results

LTO gain is the paired per-repetition ratio, >1 meaning LTO helped (rate
with / rate without for PIC, time without / time with for BS-SOLCTRA). MAPE
follows the deck's definition, averaged over the three node counts.

| PIC, LTO gain                | 1 node     | 2 nodes    | 4 nodes    |
|------------------------------+------------+------------+------------|
| C, strong (range of styles)  | 1.02-1.04  | 1.00-1.06  | 1.03-1.10  |
| C, weak                      | 1.04       | 1.02-1.04  | 1.04-1.06  |
| Rust, strong                 | 0.94-1.01  | 0.98-1.05  | 0.95-1.03  |
| Rust, weak                   | 1.00-1.01  | 1.01-1.03  | 1.01-1.02  |

| MAPE Rust vs C (%) | Geometric | Sinusoidal | Linear | Patch |
|--------------------+-----------+------------+--------+-------|
| strong, no LTO     |       2.9 |       17.4 |   23.3 |   8.5 |
| strong, LTO        |       3.2 |       20.3 |   28.3 |   5.3 |
| weak, no LTO       |       2.8 |       18.7 |   34.9 |   9.3 |
| weak, LTO          |       3.5 |       20.2 |   37.1 |   6.1 |

BS-SOLCTRA: LTO gain 1.000-1.004 for both languages at every node count;
Rust and C++ are within 1% of each other (480 s vs 476 s at 1 node, 120 s
at 4 nodes).

The Linear weak-scaling gap does not move toward the Patch residual when
both sides have LTO: it goes from 34.9% to 37.1%, because C gains 4-6%
from LTO and Rust 1-2%. Only Patch narrows (9.3% to 6.1%), which is the
style where Rust is already ahead. So LTO alone does not support the
"un-inlined RSMPI wrappers" explanation; the gap sits elsewhere.
